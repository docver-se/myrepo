...
export default function DomainSection({
  ...
  const [displayValue, setDisplayValue] = useState<string>("docver.se");
  ...
  const isEditingCustomDomain =
    editLink && data.domain && data.domain !== "docver.se" ? true : false;
  ...
  const handleDomainChange = (value: string) => {
    ...
    if (isEditingCustomDomain && !canChangeCustomDomain) {
      setDisplayValue(data.domain ?? "docver.se");
      return;
    }
    ...
    if (value === "add_domain" || value === "add_dataroom_domain") {
      setModalOpen(true);
      setData({ ...data, domain: "docver.se" });
      setDisplayValue("docver.se");
      return;
    }

    // Check if this is a custom domain selection (not docver.se)
    if (value !== "docver.se") {
  ...
  const defaultDomain = editLink
    ? (data.domain ?? "docver.se")
    : (linkType === "DOCUMENT_LINK" && canUseCustomDomainForDocument) ||
        (linkType === "DATAROOM_LINK" && canUseCustomDomainForDataroom)
      ? (domains?.find((domain) => domain.isDefault)?.slug ?? "docver.se")
      : "docver.se";
  ...
  return (
    <>
        ...
        >
          <SelectTrigger
            className={cn(
              ...
              data.domain && data.domain !== "docver.se"
                ? ""
          ...
          <SelectContent className="flex w-full rounded-md border border-input bg-white text-foreground placeholder-muted-foreground focus:border-muted-foreground focus:outline-none focus:ring-inset focus:ring-muted-foreground dark:border-gray-500 dark:bg-gray-800 focus:dark:bg-transparent sm:text-sm">
            <SelectItem value="docver.se" className="hover:bg-muted">
              docver.se
            </SelectItem>
            {linkType === "DOCUMENT_LINK" && (
              <>
        ...
        {data.domain && data.domain !== "docver.se" ? (
          <Input
            ...
            className={cn(
              ...
              data.domain && data.domain !== "docver.se" ? "flex" : "",
      ...
      {data.domain && data.domain !== "docver.se" && !isDomainVerified ? (
